...
def show_dashboard():
    ...
    if graph_options == "Lijngrafiek":
        ...
        st.sidebar.markdown("### Kleurinstellingen voor sin(x)")
        sin_color = st.sidebar.radio("Kies een kleur", ["rood", "blauw", "groen", "geel", "oranje"])

        st.sidebar.markdown("### Kleurinstellingen voor cos(x)")
        cos_color = st.sidebar.radio("Kies een kleur", ["rood", "blauw", "groen", "geel", "oranje"])

        # Koppel kleuren aan matplotlib kleuren
        color_mapping = {"rood": "red", "blauw": "blue", "groen": "green", "geel": "yellow", "oranje": "orange"}
        
        fig_line_chart = plt.figure()
        plt.plot(x, np.sin(x), color=color_mapping[sin_color], label='sin(x)')
        plt.plot(x, np.cos(x), color=color_mapping[cos_color], label='cos(x)')
        plt.legend()
